...
def plot_stats(stats_df):
    ...
    for degree, group in stats_df.groupby("degree"):
        ...
        ax.set_title(f"[OMP Poly mult] Execution Time vs threads (degree={degree:,})")
        ...
        out_path = os.path.join(PLOTS_DIR, f"omp_poly_time_vs_threads_deg{degree//1000}K.svg")
        ...
        ax.set_title(f"[OMP Poly mult] Speedup vs threads (degree={degree:,})")
        ax.grid(True, linestyle="--", alpha=0.6)
        ax.legend()
        fig.tight_layout()

        out_speed = os.path.join(PLOTS_DIR, f"omp_poly_speedup_deg{degree//1000}K.svg")
